...
config['fair_sup_parameters']['model_file'] = config['base_directory'] .. config['fair_sup_parameters']['model_file']
config['base_directory'] = config['base_directory'] .. '/fair_sup/'
...
KLD = context['KLD']
KLD.sizeAverage = config['fair_sup_parameters']['size_average']
KLD.lambda = config['fair_sup_parameters']['prior_weight']
...
-- !
if config['fair_sup_parameters']['centering'] then
...
if config['fair_sup_parameters']['noise_level'] ~= nil and config['fair_sup_parameters']['noise_level'] > 0 then
  noise = nn.PerChannelNonLinearity()
  noise.layers = {nn.SaltPepperNoise(), nn.GaussianNoise() }
  noise.layers[1].p = config['fair_sup_parameters']['noise_level']
  noise.layers[2].p = config['fair_sup_parameters']['noise_level']/2
  encoder:insert(noise, 1)
  print('[Training] using noise ' .. config['fair_sup_parameters']['noise_level'])
end
...
-- Initialize weights.
lib.init(model, config['fair_sup_parameters']['weight_initialization'], config['fair_sup_parameters']['weight_value'],
  config['fair_sup_parameters']['bias_initialization'], config['fair_sup_parameters']['bias_value'])
model = model:cuda()
print(model)

-- Criterion.
occCriterion = nn.BCECriterion()
occCriterion.sizeAverage = config['fair_sup_parameters']['size_average']
occCriterion = occCriterion:cuda()

sdfCriterion = nn.FixedVarianceGaussianNLLCriterion()
sdfCriterion.sizeAverage = config['fair_sup_parameters']['size_average']
...
optimizer = optim.sgd
if config['fair_sup_parameters']['optimizer'] then
  if config['fair_sup_parameters']['optimizer'] == 'sgd' then
    print('[Training] using sgd')
  elseif config['fair_sup_parameters']['optimizer'] == 'adam' then
    optimizer = optim.adam
    print('[Training] using adam')
  elseif config['fair_sup_parameters']['optimizer'] == 'rmsprop' then
...
batchSize = config['fair_sup_parameters']['batch_size']
learningRate = config['fair_sup_parameters']['learning_rate']
momentum = config['fair_sup_parameters']['momentum']
weightDecay = config['fair_sup_parameters']['weight_decay']
epochs = config['fair_sup_parameters']['epochs']
iterations = epochs*math.floor(N/batchSize)
minimumLearningRate = config['fair_sup_parameters']['minimum_learning_rate']
learningRateDecay = config['fair_sup_parameters']['decay_learning_rate']
maximumMomentum = config['fair_sup_parameters']['maximum_momentum']
momentumDecay = config['fair_sup_parameters']['decay_momentum']
decayIterations = config['fair_sup_parameters']['decay_iterations']
...
-- Save the model.
modelFile = config['fair_sup_parameters']['model_file']
torch.save(modelFile, model)

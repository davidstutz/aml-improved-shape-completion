...
if config['prior_parameters']['data_augmentation'] then
  dataAugmentation = config['prior_parameters']['data_augmentation']
end
...
for t = 1, iterations do
  ...
  if t%testIterations == 0 or t == 1 or t == iterations then
    ...
    for b = 0, valNumBatches do
      ...
      accValPreds_ = appendTensor(accValPreds_, valPreds)
      ...
      valProtocol[valIteration][12] = valProtocol[valIteration][12] + torch.mean(logvar.output)
    ...
    accValPreds_ = accValPreds_:narrow(1, 1, valN)
    ...
    local predFile = config['test_directory'] .. t .. '_predictions_.h5'
    lib.utils.writeHDF5(predFile, accValPreds_)
    print('[Training] ' .. t .. '|' .. iterations .. '|' .. time.hour .. ':' .. time.min .. ':' .. time.sec ..': saved ' .. predFile)

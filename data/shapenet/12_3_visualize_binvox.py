...
if __name__ == '__main__':
    ...
    for config_file in config_files:
        ...
        inputs = read_hdf5(common.filename(config, 'input_file'))
        ...
        for i in range(N):
            ...
            output = filled[n][0].copy()
            output[inputs[n][0] > 0] = 0
            load_volume(output, voxel_size, output_material, (0, 0, 0), (width*scale, depth*scale, height*scale), 'zxy')
            load_volume(inputs[n][0], voxel_size, input_material, (0, 0, 0), (width*scale, depth*scale, height*scale), 'zxy')

            rotation = (5, 0, -55)
            distance = 0.35
            png_file = vis_directory + '/%d_bin.png' % n

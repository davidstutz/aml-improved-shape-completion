...
if __name__ == '__main__':
    ...
    for config_file in glob.glob(config_folder + '/*.json'):
        ...
        base_directory = config['splitted_directory'] + '/' + set + '/'
        assert os.path.exists(base_directory), 'directory %s does not exist' % base_directory

        multiplied_directory = config['multiplied_directory'] + '/' + set + '/'
        utils.makedir(multiplied_directory)
        ...
        for n in range(len(off_files)):
            ...
            for j in range(config['multiplier']):
                ...
                out_file = multiplied_directory + '/%d.off' % i

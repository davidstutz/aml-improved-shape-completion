...
public:
  ...
  void voxelize_SCALED(int n, Eigen::Tensor<float, 4, Eigen::RowMajor>& sdf,
      Eigen::Tensor<int, 4, Eigen::RowMajor> &occ) {
    ...
    #pragma omp parallel
    {
      ...
      for (int i = 0; i < height*width*depth; i++) {
        ...
        for (unsigned int f = 0; f < this->faces.size(); ++f) {
          ...
          intersect = triangle_box_intersection(voxel_box, v1, v2, v3);
...
int main(int argc, char** argv) {
  ...
  if (!boost::filesystem::is_regular_file(config_file)) {
    std::cout << "[Error] Config file not found: " << config_file.string() << std::endl;
    exit(1);
  }

  std::ifstream json_file(config_file.string(), std::ifstream::in | std::ifstream::binary);
  ...
  #pragma omp parallel
  {
    ...
    for (unsigned int i = 0; i< indices.size(); i++) {
      ...
      mesh.scale(Eigen::Vector3f(scale, scale, scale));
      ...
      timer.start();
      mesh.voxelize_SCALED(i, sdf, occ);
      timer.stop();

      std::cout << "[Data] voxelized " << n << "/" << files.size() << " " << files[n] << " (" << timer.getElapsedTimeInSec() << ")" << std::endl;

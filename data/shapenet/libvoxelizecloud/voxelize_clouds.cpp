...
int main(int argc, char** argv) {
  ...
  std::cout << "[Data] reading " << argv[1] << std::endl;
  ...
  std::string off_directory = json.get_string("multiplied_directory");
  if (off_directory.empty()) {
    std::cout << "[Error] Read invalid off directory" << std::endl;
    exit(1);
  }

  boost::filesystem::path config_file(argv[1]);
  std::string set = config_file.stem().string();
  off_directory = off_directory + "/" + set + "/";
  ...
  std::cout << "[Data] reading " << off_directory << std::endl;
  std::map<int, std::string> files;
  read_directory(off_directory, files);

  int N = files.size();
  std::cout << "[Data] setting up depth_maps: " << N << " " << n_observations << " " << image_height << " " << image_width << std::endl;
  Eigen::Tensor<float, 4, Eigen::RowMajor> depth_maps(N, n_observations, image_height, image_width);
  depth_maps.setZero();

  std::cout << "[Data] reading " << depth_file << std::endl;
  read_hdf5<4>(depth_file, depth_maps);

  std::cout << "[Data] setting up angles: " << N << " " << n_observations << " " << 3 << std::endl;
  Eigen::Tensor<float, 3, Eigen::RowMajor> angles(N, n_observations, 3);
  angles.setZero();

  std::cout << "[Data] reading " << angles_file << std::endl;
  read_hdf5<3>(angles_file, angles);

  std::cout << "[Data] setting up K " << angles_file << std::endl;
  ...
  std::cout << "[Data] setting up mesh_center " << angles_file << std::endl;

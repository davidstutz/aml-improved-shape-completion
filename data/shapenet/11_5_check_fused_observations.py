...
if __name__ == '__main__':
    ...
    for config_file in config_files:
        ...
        inputs_tsdf = utils.read_hdf5(filename(config, 'input_ltsdf_file', num))
        print('[Data] read ' + filename(config, 'input_ltsdf_file', num))
        ...
        print('[Data] inputs_tsdf: %s' % ' x '.join(map(str, inputs_tsdf.shape)))
        ...
        for i in range(min(25, outputs.shape[0])):
            ...
            for k in range(n_observations):
                ...
                visualization.plot_specific_slices([inputs_tsdf[n][k]], slices, '%s_%d_%d_ltsdf.png' % (set, n, k), 0, 0, 3)

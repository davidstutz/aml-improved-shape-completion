...
if __name__ == '__main__':
    ...
    for config_file in os.listdir(config_folder):
        ...
        for n in range(len(off_files)):
            ...
            angles = []
            y_rotation_step = config['y_rotation_step']

            for angle in range(0, 360 - y_rotation_step, y_rotation_step):
                angles.append(angle)
            ...
            for angle in angles:
                ...
                rotation = [
                    ...
                    (angle + np.random.random()*2*config['max_y_rotation'] - config['max_y_rotation'])/180.0*math.pi,
                    (np.random.random()*2*config['max_z_rotation'] - config['max_z_rotation'])/180.0*math.pi
                ]
                mesh_prime.rotate(rotation)

                if 'x_rotation' in config:
                    rotation = [
                        config['x_rotation']/180.0*math.pi,
                        0,
                        0
                    ]
                    mesh_prime.rotate(rotation)

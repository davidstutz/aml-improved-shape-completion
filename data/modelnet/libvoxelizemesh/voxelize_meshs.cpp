...
public:
   ...
   */
  void voxelize_sdf_SCALED(int n, Eigen::Tensor<float, 4, Eigen::RowMajor>& sdf) {
   ...
   * \param[in] dense dense pre-initialized volume
   */
  void voxelize_occ_SCALED(int n, Eigen::Tensor<int, 4, Eigen::RowMajor>& occ) {

    int height = occ.dimension(1);
    int width = occ.dimension(2);
    int depth = occ.dimension(3);
    ...
    #pragma omp parallel
    {
      ...
      for (int i = 0; i < height*width*depth; i++) {
        ...
        for (unsigned int f = 0; f < this->faces.size(); ++f) {
          ...
          bool intersect = triangle_box_intersection(voxel_box, v1, v2, v3);
...
int main(int argc, char** argv) {
  ...
  bool sdf_or_occ = json.get_bool("sdf");
  ...
  #pragma omp parallel
  {
    ...
    for (unsigned int i = 0; i< indices.size(); i++) {
      ...
      if (sdf_or_occ) {
        ...
        timer.start();
        mesh.voxelize_sdf_SCALED(i, sdf);
        mesh.voxelize_occ_SCALED(i, occ);
        timer.stop();
      }
      else {
        timer.start();

...
--- Merge two tables.
-- @see https://stackoverflow.com/questions/1283388/lua-merge-tables
-- @param t1 first table
-- @param t2 secnd table
-- @return merged table
...
-- @þaram path path to file or directory
-- @return true if exists
function utils.fileOrDirectoryExists(path)
  local attr = lfs.attributes(path)
  if attr then
    if attr['mode'] == 'directory' or attr['mode'] == 'file' then
      return true
    end
  end
...
--
function utils.format_num(amount, decimal, prefix, neg_prefix)
  ...
  formatted = utils.comma_value(famount)
